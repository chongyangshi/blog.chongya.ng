...
I am not particularly good with network engineering, and since the above is not done very often, I decided not to attempt a potentially better solution bridging WireGuard interfaces of terminal instances with their local network interfaces. Because these packets have internal sources and destinations which are supposed to be routable without any NAT'ing, I am sure that the better solution would have worked given enough effort.
